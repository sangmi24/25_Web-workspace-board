...
public class ThumbnailInsertController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// multipart/form-data 로 전송되었는지부터 검사
		if(ServletFileUpload.isMultipartContent(request)) {
			...
			// Attachment 테이블에 Insert 할 데이터 뽑기
			// 단, 여러개의 첨부파일이 있을 예정이기 때문에
			// Attachment 들을 ArrayList 에 담기
			...
			for(int i = 1; i<=4 ; i++) {
				...
				//수정명=> multiRequest.getFilesystemName(key)
				...
				if(multiRequest.getOriginalFileName(key) != null ) {
					...
					list.add(at);  // 첨부파일의 갯수만큼 담김
				}	
			}
		   // 이 시점까지 도달했다면 list 에 첨부파일 객체가 다 담겨있을 것임
			int result = new BoardService().insertThumbnailBoard(b,list);
			
			if(result >0 ) { //성공 => list.th 요청
