...
public class BoardUpdateController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(ServletFileUpload.isMultipartContent(request)) {
			...
			// =>HttpServletRequest 객체를 MultipartRequest형식으로 변환
			// => 파일명 수정
			// => 서버로 업로드 
			MultipartRequest multiRequest = new MultipartRequest(request,savePath, maxSize,"UTF-8", new MyFileRenamePolicy());
			// 서버에 파일이 저장됨
			
			//3. 본격적으로 SQL문을 실행할 때 필요한 요청 시 전달값들을 뽑아서 변수에 기록 및 VO 객체로 가공
			...
			if(multiRequest.getOriginalFileName("reUpfile") != null ) {
				...
				// 수정하는 시점에서 새로이 올라온 파일에 대한 정보들
				// 전달받은 첨부파일이 있다면
				// Attchment 관련한 쿼리문들 중에서 공통적으로 필요한 값들만 우선적으로 가공
				//원본명, 수정명, 저장경로
				at = new Attachment();
			...
			// 모두 한개의 트랜잭션으로 처리하기 위해 이 지점에서 Service로 요청
			int result = new BoardService().updateBoard(b,at);
			
			// 결과에 따른 응답화면 지정
			if(result > 0) { //성공 => 해당 글의 상세보기 페이지로 (detail.vo?bno=x)
