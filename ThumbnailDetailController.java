...
public class ThumbnailDetailController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 쿼리스트링을 명시적으로 넘겼기 때문에 Get 방식=> 인코딩 설정패싱
		...
		if(result >0) { //조회수 증가 성공
			...
			// 기존의 쿼리문은 INNER JOIN 을 했었지만
			// 사진게시판에서는 카테고리 정보가 NULL이기 때문에 INNER JOIN 시 조회가 되지 않는 이슈 발생
			//=> 카테고리 컬럼 기준으로 일치하는 컬럼도, 일치하지 않는 컬럼도 가져오게끔 수정하기 위해서
			// LEFT JOIN 으로 변경시켜줌
			// (수정하더라도 일반게시판 상세조회시 문제없음, 사진게시판 상세조회시에도 활용 가능)
